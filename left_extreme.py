...
#################
lines = np.array([line5, line6, line7])
...
counter = 0
car_count = 0
start_pt = [[396, 354], [630, 312]]
end_pt = [[333, 202], [421, 173]]
num_region = 10
...
def find_equal_line(start_pt, end_pt, y):
    # takes a point on the left lane and generates a point on the right lane
    ...
    x0 = int((b2 - b1) / (m1 - m2))
    y0 = int(m1 * x0 + b1)
    # starts with a point on the left lane
    x = int((y - b1) / m1)
...
def draw_region(frame, start_pt, end_pt, regions=3):
    ...
    for step in np.arange(0, boxLength + eps, boxLength // num_regions):
        region_pts = find_equal_line(start_pt, end_pt, start_pt[0][1] - step)
...
def shortest_distance(left_extreme, lines):
    """
    This function is to find the tangent point
    :param left_extreme:
    :param lines:
    :return:
    """
    ...
    for i, line in enumerate(lines):
        tan_point = point_on_tan_line(left_extreme, line)
        # remove points to the left of the extreme points
        if (tan_point[0] - left_extreme[0]) > 0:
            pass
        else:
            uV = (tan_point[0] - left_extreme[0], tan_point[1] - left_extreme[1])
...
def main():
        ...
        for (i, contour) in enumerate(contours):
            ...
            if not contour_valid:
                ...
                if center is not None:
                    ...
                    # 2. get the index of the lowest x value (aka left extreme)
                    indice = np.argmin(contour[:, :, 0])
                    # left_extreme x coordinate
                    left_extreme_x = contour[indice][0][0]
                    # left_extreme coordinate
                    left_extreme = contour[indice][0]
                    if left_extreme_x < 320:
                        pass
                    else:
                        cv2.circle(frame, (left_extreme[0], left_extreme[1]), 4, (0, 0, 255), -1)
                        lane_index, tan_point, distance = shortest_distance(left_extreme, lines)
                        line_index = check_region(left_extreme, start_pt, end_pt, num_region)
    ...
    df.to_csv(path_or_buf='Resources/output.csv', index=False)
